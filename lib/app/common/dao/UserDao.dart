...
class UserDao {
    static login(userEmail, password, store) async {
        String signIn = userEmail + ":" + password;
        var bytes = utf8.encode(signIn);
        var base64Str = base64.encode(bytes);
        if (Config.DEBUG) {
        print("base64Str login " + base64Str);
        }
        await LocalStorage.save(Config.USER_NAME_KEY, userEmail);
        await LocalStorage.save(Config.USER_BASIC_CODE, base64Str);

        Map params = { "email": userEmail, "password": password };

        HttpManager.clearAuthorization();

    var res = await HttpManager.netFetch(Address.login(), params, null, null);
    var resultData = null;
    debugger(when: true);
